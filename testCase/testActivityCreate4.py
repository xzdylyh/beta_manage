...
import unittest
import ddt
from pages.activityCreate4Page import ActivityCreate4
...
@ddt.ddt
class TestActivityCreate4(unittest.TestCase):
    """给活跃会员赠券"""
    @classmethod
    def setUpClass(cls):
        cls.driver = select_Browser_WebDriver()
        cls.url = 'http://manage.beta.acewill.net/activity/create/4'
    ...
    @ddt.data(*get_data('activityCreate4', 'CASE1'))
    @reply_case_fail(num=3)
    def testcase1(self, data):
        """给活跃会员赠券"""
        print('========★{}★========'.format(data['case_desc'])) #case描述
        #实例化page对象
        ts = ActivityCreate4(self.url, self.driver, data['Title'])
        # 打开给活跃会员赠券页面
        ts.open
        #活动名称
